...
class Problems:
    ...
    def get_problem(level: int) -> Tuple[List[np.ndarray], List[np.ndarray], str, bool, int]:
        """Returns (inputs, targets, name, is_temporal, seq_length)."""
        problems = [
        ...
        return Problems._parity(min(level - 6, 6))
...
class YDNAEvolution:
    ...
    def run_generation(self) -> dict:
        ...
        # Dynamic early termination threshold: bottom 20% of last gen's fitness
        # or a reasonable floor if first generation
        if self.best_fitness_ever > 0:
            early_kill_threshold = self.best_fitness_ever * 0.85  # 85% of best = hopeless
        else:
        ...
        for i, chrom in enumerate(self.population):
            ...
            is_young = (chrom.age <= 2)  # grace period for new structures

            if is_elite_slot and chrom.fitness > 0:
                # Elite with cached fitness: skip re-evaluation
                continue

            # Allow early exit for established non-elites
            allow_early = not is_elite_slot and not is_young and chrom.age > 3
            chrom.fitness = self.evaluate(chrom, self.current_level, allow_early_exit=allow_early)

            # Early termination replacement: if established and hopeless, replace
...
# ============================================================
def run(max_gen_per_level: int = 500, max_level: int = 20):
    engine = YDNAEvolution(pop_size=200)
...
if __name__ == "__main__":
    random.seed(42)
    np.random.seed(42)
    engine = run(max_gen_per_level=800, max_level=20)